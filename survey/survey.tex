...
\quad Many social media interactions can be represented using networks with attributed-edges.
...
For example, in e-commerce networks, edges indicate how and when users rated products; edges in unipartite social networks contain information about when friendships were formed and when users communicated with each other.
EdgeCentric uses the information on edges for identifying suspicious nodes.

\subsubsection*{Model}

\quad The authors assume that the multiple attributes on edges, the multiple relations between the nodes, and the multiple distributions of typical human behavior are independent of each other.
Under this simplifying assumption, they define $\delta(.)$ as the anomaly function for scoring each node.
For a certain node $v$ in the underlying graph, $\delta(v)$ is defined as:
$$\delta(v) = \sum ... $$ % didn't understand what was written here on google doc!
