...
\quad Unlike EdgeCentric, FocusCo is based on the node-attribted graphs.

\subsubsection*{Approach}
\quad The exisiting techniques of clustering and outlier detection in node-attributed graphs do not incorporate the user preference. 
They either use all the attributes or perform unsupervised feature selection.
The users cannot control the relevance of the attributes and hence the results of graph mining.
In attributed graphs, focussed clustering is often of particular interest where users might be concerned with a few attributes. 
As different subsets of attributes yield different clusters, the users should be able to steer the clusters acordingly.

\subsubsection*{Model}
\quad Given a node-attributed graph $G(V,E,F)$ with $|V|=n$ nodes and $|E|=m$ edges, where each node has $|F| = d$ attributes, the the user $u$ wants to extract only the clusters of his interest. 
To do so, he provides a small set $C_{ex}$ of exemplar nodes that he considers to be similar to the type of nodes the clusters of his interest should contain. 
...
Thus, $\beta_u$ tends to be a sparse vector with large weights for only a few attributes called the focus attributes. 

\quad Once the $\beta_u$ is inferred, the focussed clusters $C$ are extracted. 
The clusters are $(1)$  dense in graph structure and separated from rest of the graph $(2)$ coherent on focussed attributes with large weights. 
The focussed clusters can be overlapping, sharing several of their nodes. 
The other objective is to detect outliers. 
Outliers $O$ are defined to be the nodes that structurally belong to a focussed cluster , but deviate from its members in some focus attributes.
...
The user initates the focussed clustering by providing $C_{ex}$. 
...
The idea in forming focussed clusters is to first find good candidate nodes and then expand around those nodes to find the clusters. 
...
Next, new nodes that increase the quality of clusters are chosen to be included in the clusters until there exist no such nodes. The quality of the a cluster $C \subset V$ is measured as its weighted conductance defined as
...
The lower the conductance of a cluster, the better is its quality.

\item \emph{Focussed Outlier Detection}:
A focused cluster outlier is a node that has many edges to the members of the cluster, but deviates from the members significantly in some focus attributes. The idea is to identify during the expandion of clusters the structural nodes that are best in terms of unweighted conductance. The best nodes which were not included in the resulting focused cluster are taken as the outliers.
\end{enumerate}

\subsubsection*{Results}
FocusCo is a scalable algorithm which runs in time linear with the size of input graph. With appropriate initializations, it can very well run in sub-linear time in practice. This helps the users in obtaining clusters and identifying outliers according to the attributes of his choice.
